...
class Board:
    ...
    def evaluate(self, is_blue_turn):
        ...
        if is_blue_turn:
            h = white_checkers-blue_checkers
            h += blue_kings*3
            h += (NUMBER_OF_PIECE-white_checkers)*3
        else:
            h = blue_checkers-white_checkers
            # h -= white_kings
            # h -= (NUMBER_OF_PIECE-blue_checkers)
        return h
...
def opponent_checkers_left(board, is_blue_turn):
    opponents_checkers = 0
    for row in range(BOARD_WIDTH):
        for col in range((row + 1) % 2, BOARD_WIDTH, 2):
            if is_blue_turn:
                if board.board[row][col].is_white():
                    opponents_checkers += 1
            else:
                if board.board[row][col].is_blue():
                    opponents_checkers += 1
    return opponents_checkers


def minimax_a_b(board, depth, move_max):
    best_move = []
    e = minimax_a_b_recurr(deepcopy(board), depth, move_max, -math.inf, math.inf, best_move)
    return best_move[-1]


def minimax_a_b_recurr(board, depth, move_max, a, b, best_move):
    # Blue player will be max player
    ...
    if move_max:
        ...
        for move in possible_moves:
            ...
            if opponent_checkers_left(tmp_board, move_max) == 0:
                return 100
            if depth == 0:
                return tmp_board.evaluate(move_max)
            value = minimax_a_b_recurr(tmp_board, depth-1, False, a, b, best_move)
            if value > best_value:
                best_value = value
        ...
        for move in possible_moves:
            ...
            if depth == 0:
                return tmp_board.evaluate(move_max)
            value = minimax_a_b_recurr(tmp_board, depth-1, True, a, b, best_move)
            if value < best_value:
                best_value = value
                if depth == MINIMAX_DEPTH:
                    best_move.append(move)
